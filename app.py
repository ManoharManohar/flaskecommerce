...
def admincreate():
    if request.method=='POST':
        ...
        print(request.form)         #accessing data from frontend as immutable dictionary format by name attribute,command prompt
        try:
            ...
            if email_count[0]==0:
                otp=genotp()                #taking from out ,created otp function,should import from that file

                admindata={'aname':aname,'aemail':aemail,'password':password,'address':address,'accept':accept,'aotp':otp}  #from one fn to another and data is stored in dictionary

                subject='Ecommerce Verifcation Code'  #for mail
                body=f'Ecommerce OTP for admin registration {otp}'
                sendmail(to=aemail,subject=subject,body=body)  #keyword argument passing ,from code , generated otp is passed in next function so paotp=otp
                flash('Otp has sent to given mail')
                return redirect(url_for('aotp',paotp=encode(data=admindata)))  #passing ecncrpyted otp // comparing data in form of dictionary
...
def aotp(paotp):
    if request.method=='POST':
        ...
        try:
            dotp=decode(data=paotp) #decoding the encode otp //  admindata={'aname':aname,'aemail':aemail,'password':password,'address':address,'accept':accept,'aotp':otp}
        except Exception as e:
            flash('Something went wrong!')
            return redirect(url_for('admincreate'))
        else:
            if dotp['aotp']==fotp:             #comparing the otp correct or not , f-frontend otp , p-passed otp  
                cursor=mydb.cursor(buffered=True)
        ...
        if request.method=='POST':
            ...
            try:
                ...
                if email_count[0]==1:
                    ...
                    stored_passsword=cursor.fetchone()
                    if stored_passsword[0].decode('utf-8')==password:
...
def admindashboard():
    if session.get('admin'):
        ...
        return redirect(url_for('adminlogin'))    #a samll amt of data abt user is session data
    
@app.route('/adminforgot',methods=['GET','POST'])
def adminforgot():
    if request.method=='POST':
        forgot_email=request.form['email']
        cursor=mydb.cursor(buffered=True)   #to check in database we use this
        cursor.execute('select count(email) from admincreate where email=%s',[forgot_email])
        stored_email=cursor.fetchone()
        if stored_email[0]==1:
            subject='Admin password reset link for Ecom Application'
            body=f"Click on the link for password update: {url_for('ad_password_update',token=encode(data=forgot_email),_external=True)}"  #converts to link-->encode(...), giving 2 args data & ext,ext-- blue color will come...if not there link but no color, will be like the ancor tag in html
...
def additem():
    if session.get('admin'):
        if request.method=='POST':
            ...
            drname=os.path.dirname(os.path.abspath(__file__))   #D:\fproj\ecom
            print(drname)
            static_path=os.path.join(drname,'static')   #D:\fproj\ecom\static ---there the file will be saved 
            print(static_path)
...
def delete_item(item_id):
    if session.get('admin'):
        try:
            ...
            print(e)
            flash("Couldn't not delete item")
    ...
    if session.get('admin'):
        try:
            ...
            if request.method=='POST':
                ...
                print(filename)
                if filename=='':   #not uploading image  , if new image uploaded then name will come .. if not '' it will come empty
...
def pay(itemid,name,price):
    try:
        ...
        return render_template('pay.html',itemid=itemid,order=order,name=name,price=total_price,qyt=qyt)
    except Exception as e:
